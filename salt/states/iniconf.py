# -*- coding: utf-8 -*-
'''
Manage ini/conf files configuration line
...
def __virtual__():
    '''
    Only load if the conf module is available
    '''
    return __virtualname__ if 'conf.set_option' in __salt__ else False
...
def options_present(name, sections=None):
    ...
    if __opts__['test']:
        ret['result'] = None
        ret['comment'] = ('conf file {0} shall be validated for presence of '
                          'given options under their respective '
                          'sections').format(name)
        return ret
    for section in sections or {}:
        for key in sections[section]:
            current_value = __salt__['conf.get_option'](name,
                                                       section,
                                                       key)
            if current_value == sections[section][key]:
                continue
            ret['changes'] = __salt__['conf.set_option'](name,
                                                        sections)
...
def options_absent(name, sections=None):
    ...
    if __opts__['test']:
        ret['result'] = None
        ret['comment'] = ('conf file {0} shall be validated for absence of '
                          'given options under their respective '
                          'sections').format(name)
        return ret
    for section in sections or {}:
        for key in sections[section]:
            current_value = __salt__['conf.remove_option'](name,
                                                          section,
                                                          key)
...
def sections_present(name, sections=None):
    ...
    if __opts__['test']:
        ret['result'] = None
        ret['comment'] = ('conf file {0} shall be validated for '
                          'presence of given sections with the '
                          'exact contents').format(name)
        return ret
    for section in sections or {}:
        cur_section = __salt__['conf.get_section'](name, section)
        if _same(cur_section, sections[section]):
            continue
        __salt__['conf.remove_section'](name, section)
        changes = __salt__['conf.set_option'](name, {section:
                                                    sections[section]},
                                                    summary=False)
        if 'error' in changes:
            ret['result'] = False
...
def sections_absent(name, sections=None):
    ...
    if __opts__['test']:
        ...
        return ret
    for section in sections or []:
        cur_section = __salt__['conf.remove_section'](name, section)
        if not cur_section:
            continue
