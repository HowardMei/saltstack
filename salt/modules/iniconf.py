...
def set_option(file_name, sections=None, summary=True):
    '''
    Edit an conf file, replacing one or more sections. Returns a dictionary
    ...
    sections : None
        A dictionary representing the sections to be edited conf file
    ...
    .. code-block:: bash
        ...
        salt '*' conf.set_option /path/to/ini '{section_foo: {key: value}}'
    '''
    if sections is None:
        sections = {}
    ...
    if not inifile:
        ret.update({'error': 'conf file not found'})
        return ret
    changes = {}
    err_flag = False
...
def get_option(file_name, section, option):
    ...
    .. code-block:: bash
        ...
        salt '*' conf.get_option /path/to/ini section_name option_name
    '''
    inifile = _Ini.get_ini_file(file_name)
    if inifile:
...
def remove_option(file_name, section, option):
    ...
    .. code-block:: bash
        ...
        salt '*' conf.remove_option /path/to/ini section_name option_name
    '''
    inifile = _Ini.get_ini_file(file_name)
    if inifile:
...
def get_section(file_name, section):
    ...
    .. code-block:: bash
        ...
        salt '*' conf.get_section /path/to/ini section_name
    '''
    inifile = _Ini.get_ini_file(file_name)
    if inifile:
...
def remove_section(file_name, section):
    ...
    .. code-block:: bash
        ...
        salt '*' conf.remove_section /path/to/ini section_name
    '''
    inifile = _Ini.get_ini_file(file_name)
    if inifile:
